# Pololu USB Device Software Development Kit Makefile
...
MaestroExample ?= Maestro/MaestroExample
...
Modules ?= $(UsbWrapper) $(Bytecode) $(Sequencer) $(Usc) $(UscCmd) $(MaestroExample) $(Programmer) $(PgmCmd)

#### Standard targets ###########################################################

# By default, create all targets when "make" is run.  This rule must be defined
# before the modules are loaded because it must be the first rule defined.
all: all_targets

# Type `make clean` to remove all the files generated by make.
clean:
	@rm -fv `find -iname *.mdb`
	@rm -fv $(Byproducts) $(Targets)
...
#### Target aliases #############################################################
# Feel free to add your own target aliases to this list if you
# want a quick way to compile just part of this project.

all_targets: $(Targets)

usccmd: $(UscCmd)/UscCmd.exe

maestro_example: $(MaestroExample)/MaestroExample.exe
